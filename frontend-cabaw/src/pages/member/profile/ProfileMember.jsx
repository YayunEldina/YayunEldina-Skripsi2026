...
const ProfileMember = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    no_telepon: "",
    alamat: "",
    old_password: "", // 🌟 State untuk kata sandi lama
    password: "",     // State untuk kata sandi baru
  });

  const [avatarPreview, setAvatarPreview] = useState(null);
  const [avatarFile, setAvatarFile] = useState(null);
  const [showOldPassword, setShowOldPassword] = useState(false); // 🌟 State mata untuk kata sandi lama
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      setFormData({
        id_pelanggan: storedUser.id_pelanggan || "",
        nama_pelanggan: storedUser.nama_pelanggan || "",
        user_name: storedUser.username || storedUser.user_name || "",
        jenis_kelamin: storedUser.jenis_kelamin || "Laki-laki",
        no_telepon: storedUser.no_telepon || "",
        alamat: storedUser.alamat || "",
        old_password: "", // Tetap kosongkan di awal halaman dimuat
        password: "",     // Tetap kosongkan di awal halaman dimuat
      });
  ...
  const handleSubmit = async (e) => {
    ...
    // 🌟 Validasi Validitas: Jika kata sandi baru diisi, kata sandi lama wajib ada.
    ...
    dataToSend.append("nama_pelanggan", formData.nama_pelanggan);
    dataToSend.append("username", formData.user_name);
    ...
    // 🌟 Kirimkan data password lama dan baru ke Laravel backend
    if (formData.old_password) {
      dataToSend.append("old_password", formData.old_password);
    }
    ...
    try {
      ...
      if (response.data && response.data.user) {
        ...
        const updatedUser = {
          ...oldUser,
          id_pelanggan: updatedUserFromBackend.id_pelanggan,
          nama_pelanggan: updatedUserFromBackend.nama_pelanggan,
          username: updatedUserFromBackend.username,
          user_name: updatedUserFromBackend.username, 
          jenis_kelamin: updatedUserFromBackend.jenis_kelamin,
          no_telepon: updatedUserFromBackend.no_telepon,
          alamat: updatedUserFromBackend.alamat,
          foto_profil: updatedUserFromBackend.foto_profil,
        };
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="pt-24 px-12 pb-16 max-w-7xl mx-auto font-sans">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select
                ...
                onChange={handleChange}
                required
                className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#1E3A5F]/20 focus:border-[#1E3A5F] transition"
              >
          ...
          {/* 🌟 BAGIAN INPUT KATA SANDI (GRID 2 KOLOM) 🌟 */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
